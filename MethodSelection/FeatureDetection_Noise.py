...
for detector in feature_detectors:
    ...
    for img in images:
        ...
        noise_list = [0.01, 0.02, 0.03, 0.04]
        ...
        for noise in noise_list:
            ...
            for i in range(5):
                ...
                #imTransformed = skimage.util.random_noise(imReference, mode='s&p', amount = noise)


                imTransformed = sp_noise(imReference, noise)
                ...
                path = os.getcwd() + "\FeatureDetectionNoise\{0}\{1}\{2}".format(detector['detector_name'],
                                                                                 str(noise), str(i))
                ...
                cv2.imwrite('{0}/Matches_{1}Noise_{2}Detector.jpg'.
                            format(path, noise, detector['detector_name']), img3)
                cv2.imwrite('{0}/Transform{1}Noise_{2}Detector.jpg'.
                            format(path, noise, detector['detector_name']), im1Reg_final)
                cv2.imwrite('{0}/imTransformed{1}Noise_{2}Detector.jpg'.
                            format(path, noise, detector['detector_name']), imTransformed)
                cv2.imwrite('{0}/imReg{1}Noise_{2}Detector.jpg'.
                            format(path, noise, detector['detector_name']), imReg)

                average_distance, successful, im_verification = verification(imReg, imTransformed, detector)

                cv2.imwrite('{0}/verification_matches_{1}Noise_{2}Detector_'
                            '{3}Success.jpg'.format(path, noise, detector['detector_name'], successful), im_verification)
                ...
                values.append([img.split('/')[3], i, 0, thetaRecovered, average, len(kp_1), len(kp_2),
                               ...
                               average_distance, noise, 1, scale_recovered])

                print('File: {}; Scale: {}; Scale_recovered: {}; Detector: {}; Compute_Time: {}'.format
                      (img.split('/')[3], round(1), scale_recovered, detector['detector_name'], elapsed_time))
...
df = pd.DataFrame.from_records(values, columns=['file_name', 'iteration', 'actual_theta(degrees)',
                                                ...
                                                'noise', 'scale', 'scale_recovered'])

df.to_csv('FeatureDetectionNoise/result_noise.csv')
full_end = time.time()
