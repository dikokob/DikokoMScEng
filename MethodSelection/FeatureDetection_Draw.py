...
for detector in feature_detectors:
    ...
    for img in images:
        ...
        imKeypoints = cv2.drawKeypoints(imReference, kp_1, imKeypoints, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)

        path = os.getcwd() + "\FeatureDetectionDraw\{0}".format(detector['detector_name'])
        ...
        cv2.imwrite('{0}\{1}'.format(path, img.split('/')[-1].replace('.pgm', 'keypoints_.png')), imKeypoints)

        cv2.imwrite('{0}\{1}'.format(path, img.split('/')[-1].replace('.pgm', 'original_.png')), imReference)
