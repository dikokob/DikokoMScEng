...
def transform(good_matches, img1, kp1, img2, kp2):
    ...
    return im1_reg_final, theta_recovered, im1_reg, x_recovered, y_recovered
    ...
    for img in images:
        ...
        for delta in range(0, 1000, 100):
            ...
            for i in range(5):
                ...
                imTransformed = cv2.warpAffine(imReference, T, (width, height))
                ...
                im1Reg_final, thetaRecovered, imReg, x_recovered, y_recovered = \
                    transform(good_matches_found, imReference, kp_1, imTransformed, kp_2)

                path = os.getcwd() + "\FeatureDetectionTranslation\{0}\{1}\{2}".format(detector['detector_name'],
                                                                                       str(delta), str(i))
                try:
                    os.makedirs(path)
                except:
                ...
                cv2.imwrite('{0}/Matches_{1}Translation_{2}Detector.jpg'.
                            format(path, delta, detector['detector_name']), img3)
                cv2.imwrite('{0}/Transform{1}Translation_{2}Detector.jpg'.
                            format(path, delta, detector['detector_name']), im1Reg_final)
                cv2.imwrite('{0}/imTransformed{1}Translation_{2}Detector.jpg'.
                            format(path, delta, detector['detector_name']), imTransformed)
                cv2.imwrite('{0}/imReg{1}Translation_{2}Detector.jpg'.
                            format(path, delta, detector['detector_name']), imReg)
                ...
                cv2.imwrite('{0}/verification_matches_{1}Translation_{2}Detector_'
                            '{3}Success.jpg'.format(path, delta, detector['detector_name'], successful), im_verification)
                ...
                values.append([img.split('/')[3], i, 0, thetaRecovered, average, len(kp_1), len(kp_2),
                               ...
                               average_distance, x_recovered, y_recovered, delta, delta])

                print('File: {}; Distance: {}; Calculated_Distance: {}; Detector: {}; Compute_Time: {}'.format
                      (img.split('/')[3], round(delta), round(x_recovered), detector['detector_name'], elapsed_time,))
...
df = pd.DataFrame.from_records(values, columns=['file_name', 'iteration', 'actual_theta(degrees)',
                                                ...
                                                'x_recovered', 'y_recovered', 'x_delta_origin', 'y_delta_origin'])

df.to_csv('FeatureDetectionTranslation/result_translation.csv')
full_end = time.time()
