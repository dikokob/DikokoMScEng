...
def main(num):
    ...
    images = [images_maze_diff_scale] # 3 [images_turtle_diffRes4, images_maze_diff_scale, images_fr0] # 2 [images_turtle_diffRes4, images_maze
        ...
        for combination in image['combinations']['combinations']:
            ...
            try:
                ...
                for i in range(1, len(img_list)):
                    ...
                    #cv2.imwrite('{0}/Matches_{1}.jpg'. format(path, combination[i]), im_inliers) TODO return
                    #cv2.imwrite('{0}/OriginalImage_{1}.jpg'. format(path, combination[i]), img_list[i]) TODO return
                    #cv2.imwrite('{0}/TransformedImage_{1}.jpg'. format(path, combination[i]), imReg) TODO return

                df_bench_marking = pd.DataFrame(bench_marking_dict_list)
                # df_bench_marking.to_csv('{0}\\bench_marking.csv'. format(path), index=False) TODO return
                ...
                for img_reg in img_reg_list:
                    ...
                    #cv2.imwrite('{0}/Final.jpg'. format(path), final_image_1) TODO return
